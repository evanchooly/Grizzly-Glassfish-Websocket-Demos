...
public class WarGameTest {
    public void duel() throws IOException, InstantiationException, InterruptedException {
        ...
        try {
            ...
            Assert.assertTrue(adapter1.getQueue().poll(10, TimeUnit.SECONDS).startsWith("placed"));
            ...
            Assert.assertTrue(adapter2.getQueue().poll(10, TimeUnit.SECONDS).startsWith("placed"));

            client1.send("strike:0:0");
            Assert.assertTrue(adapter1.getQueue().poll(10, TimeUnit.SECONDS).startsWith("boom"));
            client1.send("strike:10:10");
            Assert.assertTrue(adapter1.getQueue().poll(10, TimeUnit.SECONDS).startsWith("whiff"));
            client1.send("strike:0:1");
            Assert.assertEquals(adapter1.getQueue().poll(10, TimeUnit.SECONDS), "you win");
